...
src/
  ...
  features/studio/
    ...
    data/              # Static data (default credit packages; generated prompt gallery data)
...
- Package manager: pnpm (`pnpm-lock.yaml` is canonical; stray `bun.lock` / `package-lock.json` are not used)
...
- `src/features/studio/data/image-gallery.ts` / `video-gallery.ts` and `public/studio-showcase/gallery/` are generated by a local curation pipeline -- regenerate instead of hand-editing entries
- `/scripts` is gitignored (local verification/CDP scripts containing test credentials) -- never commit it or import from it in tracked code
- `doc/` holds upstream API reference docs (Duomi/Sora/Veo, Chinese); `docs/` holds internal design/research notes (parts gitignored)
